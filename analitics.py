...
def apply_deepfool_to_data_loader(model, orig_data, epss, num_iter, device):
    model.eval()
    results = []
    for eps in epss:
        adversarial_images = []
        ...
        for img_tensor, label in tqdm(orig_data):
            img_tensor = img_tensor.to(device)
            adv_images = deepfool(model, img_tensor, eps, num_iter, device)
            adversarial_images.append(adv_images)
            true_labels.append(label)
            count += 1
            if count >= 5:
...
def plot_five_img_adv_conf(data, pred_labels, confidences, title, labels):
    counter = 0
    plt.figure(figsize=(25, 9))
    for (img_tensor, _), label, confidence in zip(data, pred_labels, confidences):
        if counter >= 5:
            break
        adv_img = to_array(img_tensor.cpu())
        plt.subplot(1, 5, counter + 1)
...
def fgsm_analitics(device):
    ...
    for eps, adv in zip(epss, adv_data):
        accuracy, pred, change_label_num, _ = evaluate_model(model, adv)
        acc_list.append(accuracy)
...
def l_bfgs_analitics(device):
    ...
    for eps, adv in zip(epss, adv_data):
        accuracy, pred, change_label_num, confidences = evaluate_model(model, adv)
        acc_list.append(accuracy)
        plot_five_img_adv_conf(adv, pred, confidences, f"Adv Images(eps = {eps}, iters num = {num_iter})", labels)
        print(f'Точность модели на тестовом наборе при eps={eps}, и {num_iter} итерациях: {accuracy:.2f}')
        print(f'Количество изображений, поменявших свой класс: {change_label_num}')
    plot_acc_diff(acc_list, epss)



def deepfool_analitics(device):
    model = models.vgg16(weights=models.VGG16_Weights.DEFAULT).to(device)
    labels = models.VGG16_Weights.DEFAULT.meta['categories']
    model.eval()
    orig_data = load_data(device)
    ...
    epss = [5, 10, 15, 20]
    num_iter = 15
    adv_data = apply_deepfool_to_data_loader(model, orig_data, epss, num_iter, device)
    for eps, adv in zip(epss, adv_data):
        accuracy, pred, change_label_num, confidences = evaluate_model(model, adv)
        acc_list.append(accuracy)
        plot_five_img_adv_conf(adv, pred, confidences, f"Adv Images(eps = {eps}, iters num = {num_iter})", labels)
